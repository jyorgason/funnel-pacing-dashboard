...
function EditableSummary({ period }) {
  ...
  return (
    <div className="es-callout" style={{ position: "relative", display: "flex", gap: 14, alignItems: "flex-start", background: "var(--gray-05)", border: "1px solid var(--gray-2)", borderRadius: "var(--radius-300)", padding: "16px 18px" }}>
      <span style={{ marginTop: 4, width: 12, height: 12, borderRadius: "50%", background: s.fill, flexShrink: 0, display: "inline-block" }} />
      ...
      {/* hover-only provenance + controls — stays out of clean screenshots */}
      <div className="es-tools" style={{ position: "absolute", top: 12, right: 14, display: "flex", alignItems: "center", gap: 10 }}>
        <span style={{
        ...
        {edited && (
          <button onClick={reset} style={{ cursor: "pointer", border: "1px solid var(--gray-3)", background: "var(--white)", color: "var(--gray-7)", fontSize: 11, fontWeight: 600, padding: "4px 10px", borderRadius: "var(--radius-full)", fontFamily: "var(--font-sans)" }}>
            Reset to auto-draft
          </button>
        )}
      </div>
